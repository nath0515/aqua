 ...
 if (isset($_SESSION['loggedin'])) {
    ...
    if(!isset($_GET['status'])){
        if(!isset($_GET['token'])){
            ...
            $sql = "SELECT * FROM users WHERE fp_token = :fp_token";
            $stmt = $conn->prepare($sql);
            $stmt->bindParam(':fp_token', $_GET['token']);
            $stmt->execute();
            if (!($stmt->rowCount() > 0)) {
                header("Location: 401.html");
                exit();
                    ...
                    <div class="row justify-content-center">
                        <div class="col-lg-4">
                            ...
                            <img src="assets/img/logo.png" alt="Logo" class="mb-2" style="width: 150px; height: auto; display: block; margin: 0 auto;">
                                ...
                                <div class="card-body">
                                    <form action="process_forgotpassword.php" method="POST" onsubmit="return checkForm()">
                                        <input type="text" name="token" class="form-control" value="<?php if(isset($_GET['token'])) echo $_GET['token']?>" hidden>
                                        <div class="mb-3">
        ...
        <script>
            ...
            <?php elseif ($_GET['status'] == 'success'): ?>
                Swal.fire({
                    ...
                    text: 'The password has been successfully changed. You can now log in.',
